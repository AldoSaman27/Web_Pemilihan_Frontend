...
const AdminHome = () => {
  ...
  useEffect(() => {
    ...
    axios.defaults.headers.common["Accept"] = "application/json";
    axios
      .get(`${process.env.REACT_APP_API_URL}/api/v2/voting/index`)
      .then((ress) => {
  ...
  const KandidatDataChart = Array.from(
    { length: kandidatData.length },
    (_, index) => `Paslon ${index + 1}`
  );
  const data = {
    ...
    datasets: [
      {
        ...
        data: kandidatData.map((kandidat, index) => {
          return votingData.filter(
            (voting) => voting.kandidats_id === index + 1
          ).length;
        }),
        backgroundColor: [
          "#192655",
