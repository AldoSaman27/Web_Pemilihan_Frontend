...
import { Button, Container, Card, ListGroup, Modal, Form, FloatingLabel, Alert } from 'react-bootstrap';
...
const Vote = () => {
    ...
    const HandleSubmit = () => {
        if (votingCode === "" || votingCode === 0 || votingCode === undefined || votingCode === null) {
        ...
        axios.post(`${process.env.REACT_APP_API_URL}/api/v2/voting/store`, formData).then((ress) => {
    ...
    const HandleVotingCodeChange = (e) => {
        if (e.target.value === "" || e.target.value === 0 || e.target.value === null || e.target.value === undefined) {
            setAlertValue("Voting code is required!");
            setAlertIsHide(false);
            return 1;
        } 
        ...
        <>
            ...
            <section id="Vote">
                <Container className="d-flex flex-column align-items-center justify-content-center">
                    <h2 className="fw-bold text-center mt-3 text-primary">PEMILIHAN KETUA OSIS & WAKIL KETUA OSIS</h2>
                    <h2 className="fw-bold text-center text-primary">SMK NEGERI 3 GORONTALO</h2>
                    <h2 className="fw-bold text-center mb-3 text-primary">PERIODE 2024 - 2025</h2>
                    <div className="row">
                            ...
                            <div key={index} className="col m-4 d-flex align-items-center justify-content-center">
                                <Card style={{ width: '18rem' }} className="mb-3">
                                    ...
                                    <Card.Body className="d-flex align-item-center justify-content-center">
                                        <Button variant="primary" onClick={() => HandlePilih(index+1, item.id)}>Pilih</Button>
                                    </Card.Body>
                                </Card>
                            </div>
                        ))}
                        <Modal show={show} onHide={() => setShow(false)}>
                            ...
                            <Modal.Body>
                                <Alert key="danger" variant="danger" className={alertIsHide ? 'd-none' : null}>
                                    {alertValue}
                                </Alert>
                                <FloatingLabel controlId="floatingInput" label="Voting Code" className="mb-3">
                                    <Form.Control type="text" placeholder="VSG2CsFJ" onChange={HandleVotingCodeChange} />
                                </FloatingLabel>
                            </Modal.Body>
                            <Modal.Footer>
                                ...
                                <Button variant="primary" className="d-flex align-items-center justify-content-center gap-2" onClick={HandleSubmit} disabled={buttonLoading}>
                                    {buttonLoading ? (
